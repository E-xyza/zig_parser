defmodule Zig.Parser.Test.StatementTest do
  ...
  defmacrop toplevelblockcontent(to_bind) do
    quote do
      [{:comptime, _, {:block, _, [unquote(to_bind)]}}]
    end
  end
  ...
  describe "var declarations" do
    test "can be empty" do
      toplevelblockcontent(var) = Parser.parse("comptime {var x = 1;}").code
      assert {:var, _, {:x, _, {:integer, 1}}} = var
    end

    test "can be comptime" do
      toplevelblockcontent(var) = Parser.parse("comptime {comptime var x = 1;}").code
  ...
  describe "const declarations" do
    test "can be empty" do
      toplevelblockcontent(const) = Parser.parse("comptime {const x = 1;}").code
      assert {:const, _, {:x, _, {:integer, 1}}} = const
    end

    test "can be comptime" do
      toplevelblockcontent(const) = Parser.parse("comptime {comptime const x = 1;}").code
  ...
  describe "prefixed block declarations" do
    test "can be comptime" do
      toplevelblockcontent(comptime) = Parser.parse("comptime {comptime {}}").code
      assert {:comptime, _, {:block, _, []}} = comptime
    end
