defmodule Zig.Parser.Test.TopLevelVarTest do
  ...
  describe "when given a top level basic var block" do
    # tests:
    # TopLevelVar <- (KEYWORD_export / KEYWORD_extern STRINGLITERALSINGLE?)? KEYWORD_threadlocal? VarDeclProto
  ...
  describe "when given contents of the var decl" do
    # tests:
    # VarDeclProto <- (KEYWORD_const / KEYWORD_var) IDENTIFIER (COLON TypeExpr)? ByteAlign? LinkSection? (EQUAL Expr)? SEMICOLON
  ...
  describe "multi-assignment" do
    test "can assign multiple consts" do
      ...
      assert [
             ...
             ] = Parser.parse("comptime { const result, const overflow = fun(a, b);}").code
    end
  end
