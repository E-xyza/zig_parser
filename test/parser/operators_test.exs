defmodule Zig.Parser.Test.OperatorsTest do
  ...
  describe "binary operators in expressions" do
    ...
    test "or operator" do
      assert %Parser{decls: [%Const{value: {:or, %{expr: "a"}, %{expr: "b"}}}]} =
               Parser.parse("const foo = a or b;")
    end

    test "and operator" do
      assert %Parser{decls: [%Const{value: {:and, %{expr: "a"}, %{expr: "b"}}}]} =
               Parser.parse("const foo = a and b;")
    end

    test "equals operator" do
      assert %Parser{decls: [%Const{value: {:==, %{expr: "a"}, %{expr: "b"}}}]} =
    ...
    test "orelse operator" do
      assert %Parser{decls: [%Const{value: {:orelse, %{expr: "a"}, %{expr: "b"}}}]} =
  ...
  describe "unary prefix operators in expressions" do
    test "boolean negation operator" do
      assert %Parser{decls: [%Const{value: {:!, %{expr: "a"}}}]} = Parser.parse("const foo = !a;")
    end
    ...
    test "bitwise negation operator" do
      assert %Parser{decls: [%Const{value: {:"~", %{expr: "a"}}}]} =
    ...
    test "try operator" do
      assert %Parser{decls: [%Const{value: {:try, %{expr: "a"}}}]} =
               Parser.parse("const foo = try a;")
    end

    test "await operator" do
      assert %Parser{decls: [%Const{value: {:await, %{expr: "a"}}}]} =
