defmodule Zig.Parser.Test.ContainerDeclTest do
  ...
  describe "struct container decl" do
    test "can be parsed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = struct{};")
      assert {:struct, %{extern: false, packed: false}, []} = expr
    end

    test "can be extern" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = extern struct{};")
      assert {:struct, %{extern: true}, []} = expr
    end

    test "can be packed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed struct{};")
  ...
  describe "opaque container decl" do
    test "can be parsed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = opaque{};")
      assert {:opaque, %{extern: false, packed: false}, []} = expr
    end

    test "can be extern" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = extern opaque{};")
      assert {:opaque, %{extern: true}, []} = expr
    end

    test "can be packed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed opaque{};")
  ...
  describe "enum container decl" do
    test "can be parsed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = enum{};")
      assert {:enum, %{extern: false, packed: false, type: nil}, []} = expr
    end

    test "can be extern" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = extern enum{};")
      assert {:enum, %{extern: true}, []} = expr
    end

    test "can be packed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed enum{};")
      assert {:enum, %{packed: true}, []} = expr
    end

    test "can be typed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed enum(u8) {};")
  ...
  describe "union container decl" do
    test "can be parsed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = union{};")
      assert {:union, %{extern: false, packed: false, tagtype: nil}, []} = expr
    end

    test "can be extern" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = extern union{};")
      assert {:union, %{extern: true}, []} = expr
    end

    test "can be packed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed union{};")
      assert {:union, %{packed: true}, []} = expr
    end

    test "can be typed" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed union(my_enum) {};")
      assert {:union, %{packed: true, tagtype: :my_enum}, []} = expr
    end

    test "can have an inferred enum type" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed union(enum) {};")
      assert {:union, %{packed: true, tagtype: nil, tagged: true}, []} = expr
    end

    test "can have a backed, inferred enum type" do
      assert %Parser{decls: [%Const{value: expr}]} = Parser.parse("const foo = packed union(enum(u8)) {};")
